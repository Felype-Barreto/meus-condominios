-- Limit how many active condominiums one account can own by plan.

alter table public.plan_limits
  add column if not exists max_condominiums_per_account int not null default 1;

update public.plan_limits
set max_condominiums_per_account = case plan
  when 'free' then 1
  when 'premium' then 2
  when 'pro' then 5
  when 'total' then 20
...
declare
  ...
  owner_id uuid := coalesce(new.owner_user_id, auth.uid());
begin
  resolved_plan := public.get_entitled_plan_for_user(owner_id);

  select coalesce(pl.max_condominiums_per_account, 1)
    into account_limit
  from public.plan_limits pl
  where pl.plan = resolved_plan;

  select count(*)
    into active_condominiums
  from public.condominiums c
  where c.owner_user_id = owner_id
    and c.subscription_status not in ('canceled', 'blocked', 'pending_deletion');

  if active_condominiums >= account_limit then
    raise exception 'Seu plano % permite ate % condominio(s).', resolved_plan, account_limit;
  end if;
...
begin
  ...
  if active_condominiums >= coalesce(plan_row.max_condominiums_per_account, 1) then
    raise exception 'Seu plano % permite ate % condominio(s).', plan_name, plan_row.max_condominiums_per_account;
  end if;
