...
begin
  ...
  local_start := new.start_at at time zone 'America/Sao_Paulo';
  local_end := new.end_at at time zone 'America/Sao_Paulo';
  ...
  if coalesce(array_length(area_record.available_days, 1), 0) > 0
    and not (extract(dow from local_start)::int = any(area_record.available_days)) then
    raise exception 'Area indisponivel neste dia da semana.';
  end if;

  if local_start::time < area_record.available_start_time
    or local_end::time > area_record.available_end_time then
  ...
  select exists (
    ...
    where b.condominium_id = new.condominium_id
      ...
      and b.blocked_date = local_start::date
      and (
        b.full_day = true
        or (
          b.start_time is not null
          and b.end_time is not null
          and local_start::time < b.end_time
          and local_end::time > b.start_time
        )
      )
  ) into blocked_exists;
  ...
  where b.condominium_id = new.condominium_id
    ...
    and b.status in ('pending', 'approved')
    and date_trunc('month', b.start_at at time zone 'America/Sao_Paulo') = date_trunc('month', local_start);
