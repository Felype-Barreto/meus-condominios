...
export async function createResidentInviteAction(
  ...
  return {
    status: "success",
    message: "Convite criado. Este link expira em 10 minutos.",
    inviteUrl,
    expiresAt: result.expires_at,
    whatsappText: `Olá! Você recebeu um convite para se cadastrar no Meus Condomínios. Este link expira em 10 minutos e só pode ser usado uma vez: ${inviteUrl}${condo?.slug ? `\n\nCódigo do condomínio para entrar depois: ${condo.slug}` : ""}`,
  };
}
